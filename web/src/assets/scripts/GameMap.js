...
export class GameMap extends AcGameObject{
        ...
        if(this.recordStore.is_record){
            ...
            const interval_id = setInterval(() => {
                if(k >= a_steps.length - 1){
                    ...
                    if(loser === "all" || loser === "B") snake1.status = "die";
            ...
            this.ctx.canvas.addEventListener("keydown", e => {
                ...
                if(d >= 0){
                    this.pkStore.socket.send(JSON.stringify({ event: "move", direction: d }));
                }
            });
        }
    }
    ...
    check_ready(){
        for(const i of this.snakes){
            if(i.direction === -1) return false;
    ...
    render(){
        ...
        for(let r = 0; r < this.rows; r++){
            for(let c = 0; c < this.cols; c++){
                this.ctx.fillStyle = (c + r) % 2 === 0 ? color_even : color_odd;
